...
export default function FadyCalzados() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const videoIds = new Set(videoProds.map(p => p.id));
      const all = [...styloProds, ...fadyProds].filter(p => !videoIds.has(p.id));
        ...
        <div style={{position:"relative",zIndex:2,padding:"clamp(90px,15vw,130px) clamp(24px,6vw,80px) clamp(60px,10vw,100px)"}}>
          ...
          {videoProducts.length > 0 && (
            <div
              ...
              style={{display:"inline-flex",alignItems:"center",gap:14,marginTop:24,cursor:"pointer"}}
            >
              {/* Filled play circle */}
              <div style={{width:44,height:44,borderRadius:"50%",background:"#8B6240",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 4px 18px rgba(139,98,64,0.38)",transition:"transform 0.2s,box-shadow 0.2s"}}
                onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.08)";e.currentTarget.style.boxShadow="0 6px 24px rgba(139,98,64,0.55)";}}
                onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.boxShadow="0 4px 18px rgba(139,98,64,0.38)";}}>
                <div style={{width:0,height:0,borderTop:"7px solid transparent",borderBottom:"7px solid transparent",borderLeft:"13px solid #fff",marginLeft:3}}/>
              </div>
              <div>
                <div className="mt" style={{fontSize:8,letterSpacing:"0.32em",color:"#8B6240",lineHeight:1.7}}>COMPRA A TRAVÉS DE VÍDEOS</div>
                <div className="cg" style={{fontSize:14,color:"#2c1a0e",fontWeight:300,fontStyle:"italic",lineHeight:1.2}}>Ver nuestra colección</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div ref={vidGridRef} style={{background:"#0a0a0a",padding:"52px 0 40px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:3,padding:"0 3px"}}>
            {videoProducts.map((p, i) => (
              ...
              >
                {p.videos?.[0]?.preview
                  ? <img src={p.videos[0].preview} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover"}} loading="lazy"/>
                  : p.photoUrl
